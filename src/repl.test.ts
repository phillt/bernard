...
describe('REPL startup auto-create promotion', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    rlEmitter = makeRl();
    vi.spyOn(console, 'clear').mockImplementation(() => {});
  ...
  it('auto-creates pending candidates above threshold at startup when auto-create is already on', async () => {
    const { startRepl } = await import('./repl.js');
    const config = makeConfig({ autoCreateSpecialists: true, autoCreateThreshold: 0.8 });

    mockListPending.mockReturnValue([
      {
        id: 'cand-startup',
        draftId: 'personal-gmail-triage',
        name: 'Personal Gmail Triage',
        description: 'Triages personal Gmail',
        systemPrompt: 'You triage Gmail',
    ...
    expect(mockSpecialistCreate).toHaveBeenCalledWith(
      'personal-gmail-triage',
      'Personal Gmail Triage',
      'Triages personal Gmail',
      'You triage Gmail',
      ['Be concise'],
    );
    expect(mockCandidateUpdateStatus).toHaveBeenCalledWith('cand-startup', 'accepted');
    expect(mockPrintInfo).toHaveBeenCalledWith(
      expect.stringContaining('Specialist auto-created: "Personal Gmail Triage"'),
    );
  ...
  it('does not auto-create at startup when auto-create flag is off', async () => {
    const { startRepl } = await import('./repl.js');
    const config = makeConfig({ autoCreateSpecialists: false, autoCreateThreshold: 0.8 });
