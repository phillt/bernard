...
export const DOMAIN_REGISTRY: Record<string, MemoryDomain> = {
  'tool-usage': {
    ...
    extractionPrompt: `You are a tool-usage pattern extractor. Extract facts about how tools, commands, and workflows are used in the conversation below.
...
- File navigation paths and directory structures discovered
...
Return a JSON array of strings. Each string should be a self-contained fact (understandable without the original conversation). Maximum 500 characters per fact. If there are no notable facts, return an empty array [].`,
  ...
  'user-preferences': {
    ...
    extractionPrompt: `You are a user preference extractor. Extract facts about the user's preferences, habits, and conventions from the conversation below.
...
Return a JSON array of strings. Each string should be a self-contained fact (understandable without the original conversation). Maximum 500 characters per fact. If there are no notable facts, return an empty array [].`,
  ...
  'general': {
    ...
    extractionPrompt: `You are a general knowledge extractor. Extract facts about the project, environment, and context from the conversation below.
