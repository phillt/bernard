...
import { locateQuote } from '../claim-verifier.js';
  ...
  return attachMeta(
    tool({
      ...
      execute: async ({ action, id, quote }): Promise<string> => {
        ...
        if (action === 'locate') {
          if (!quote)
            return JSON.stringify({ error: 'quote is required when action is "locate".' });
          // Narrowed to one source when asked, otherwise every registered one —
          // first match wins, which is stable and needs no rule about which
          // source is "better".
          const one = id ? provenance.get(id) : undefined;
          const candidates = id ? (one ? [one] : []) : provenance.list();
          ...
          if (!hit) {
            return JSON.stringify({
              ...
              // The distinction that matters: only two of the seven producers
              // retain the full text, so for the rest "not found" can mean
              // "not found in the first 2,000 characters".
              partial: candidates.filter((s) => s.verifyText === undefined).map((s) => s.id),
            });
          }
          const source = provenance.get(hit.sourceId)!;
          const body = source.verifyText ?? source.contentPreview;
          ...
          return JSON.stringify({
            ...
            // A bounded window, never the whole retained text: withholding that
            // is the entire point of the field, and `get` strips it for the
