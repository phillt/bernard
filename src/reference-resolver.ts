...
- If NO entry matches a phrase, omit it. The agent will ask if needed.
- Generic words ("the file", "this code") that don't point at a stored memory are NOT references — omit.
...
export function shouldSkipResolver(userInput: string, memoryKeys: string[]): boolean {
  if (memoryKeys.length === 0) return true;
  if (!REFERENCE_PRESCAN.test(userInput)) return true;
  return false;
}
