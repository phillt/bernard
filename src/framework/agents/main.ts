...
export const mainAgentDefinition: AgentDefinition<MainInput, string> = {
  ...
  async tools(ctx, input, surface): Promise<Record<string, Tool>> {
    ...
    // Deferred, not static: `applet-ctx-tools.js` reaches `tools/applet.js`, and
    // a static edge from here made that module and 13 others eager for every
    ...
    const tools: Record<string, Tool> = {
      ...
      // so spread order is load-bearing: move this line last and both the
      // styling and planning passes silently disappear with no type error. See
      // `tools/applet-ctx-tools.ts` for why the ctx-carrying instance has to be
      // built here.
      ...baseTools,
