...
- `BERNARD_MCP_CONNECT_TIMEOUT_MS` — Per-server MCP connect + handshake + tool-listing timeout at startup and reconnect (default: 15000). A server that never completes the MCP `initialize` handshake (e.g. an HTTP/SSE server misconfigured as stdio) is marked failed and skipped — its spawned stdio child is killed and the REPL starts without it — instead of blocking startup forever (#254). Applies to both the REPL and the cron daemon.
